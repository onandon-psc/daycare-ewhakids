<?
	...
	if( $modeType == "id" ){
		...
		$query = "SELECT mbId FROM ona_member WHERE mbName='".$mbName."' && mbEmail='".$mbEmail."' && mbStatus != 'C'";
		...
		$query = "SELECT mbId, mbName, mbPwd, mbEmail FROM ona_member WHERE mbId='".$mbId."' && mbName='".$mbName."' && mbEmail='".$mbEmail."' and mbStatus != 'C'";
		$result	= mysql_query($query);
		$row		= mysql_fetch_array($result);
