...
<!--
	...
	function delClick(){
		...
		var check_value1 = getChecked2('1');
		var check_value2 = getChecked2('2');

		if(check_value1 == ""){
			alert("삭제 할 회원을 선택하세요!");
			return false;
		}else{
			if(confirm('정말 삭제하시겠습니까?')){
				f.delValue.value	= check_value1;
				f.delValue2.value	= check_value2;
				return;
			}else{
				return false;	
			}
	...
	function email_check(f){
		...
		if( f.emailType[0].checked == false && f.emailType[1].checked == false && f.emailType[2].checked == false ){
			alert("EMAIL을 보낼 회원분류를 선택하십시오!");
			return false;
		}		
		
		if(f.emailType[1].checked == true){
			var check_value = getChecked();
			if(check_value == ""){
				alert("EMAIL을 보낼 회원을 선택하세요!");
				return false;
			}else{
		...
		f.email_content.value = SubmitHTML();

		if( !f.email_content.value ){
			alert('EMAIL 내용을 입력하세요!');
			//f.email_content.focus();
			return false;
		}

		if(confirm('EMAIL을 발송 하시겠습니까?')){		
			return;
		}else{
			return false;
		}
...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
  <tr height=30>
    <td bgcolor=#ececec>&nbsp;&nbsp;<b>▣ 회원 이메일발송</b></td>
  </tr>
  <tr>
    <td height="15"></td>
  </tr>
  ...
  <form name="frm" method="post" action="/html/member/member_proc.php" onsubmit="return input_check(this);" target="iframe">
          ...
          <td>
            ...
            <table width='100%' cellpadding="5" cellspacing="0" border="0" align="center">
              ...
              <tr height="25" bgcolor="<?=$tr_bgcolor?>" onMouseOver=this.style.backgroundColor='#F5F5EA' onMouseOut=this.style.backgroundColor='<?=$tr_bgcolor?>'  align="center">
                ...
                <td><a href='<?=$PHP_SELF?>?id=<?=$row[userid]?>&search=<?=$search?>&keyword=<?=$keyword?>&page=<?=$page?>'> <b>
                  <?=$row['userid']?>
                  </b></a> </td>
                <td>
                  <?=$row['name']?>
			...
			<form name="emailFrm" method="post" action="email_proc.php" onsubmit="return email_check(this);" target="iframe">
				...
				<tr> 
				  <td width="100%" align="center">
					<table width='98%' border="0" cellpadding="0" cellspacing="4" bgcolor="#F2F3E8">
					  <tr> 
						<td width="80" align="center" bgcolor="#F5F5EA"><b>EMAIL 발송</b></td>
						<td align="center" bgcolor="#FFFFFF">
							<table width="100%" cellspacing="0" cellpadding="0" border="0">
								<tr>
									<td>
										<table width="100%" cellspacing="0" cellpadding="0" border="0">
											<tr>
												<td>
													<input type="radio" name="emailType" value="S"> 검색된 회원&nbsp;&nbsp;
													<input type="radio" name="emailType" value="C"> 선택된 회원&nbsp;&nbsp;
													<input type="radio" name="emailType" value="A"> 전체 회원
												</td>
											</tr>
										</table>
									</td>
								...
								<tr>
									<td>
										제목 : <input type="text" name="email_title" value="" style="width:80%;IME-MODE:active;" maxlength="255">
									</td>
								<tr>								
<?
include_once($_SERVER["DOCUMENT_ROOT"].'/include/editor/func_editor.php');
$upload_image = '';
$upload_media = '';
?>
								<tr>
									<td><?=myEditor('1','../../include/editor','emailFrm','email_content','100%','400','');?></td>
								<tr>								
							</table>
						</td>
						<td width="80" align="center" bgcolor="#FFFFFF"><input type="submit" value=" 발송 " style="width:100%;height:70" style="cursor:pointer"></td>
					  </tr>					  
					</table>
				  </td>
				</tr>
